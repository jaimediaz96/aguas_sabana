...
public class MicroRouteBusinessController {
    ...
    @PutMapping("/updateMicroRouteId/{microRouteId}")
    public ResponseEntity<ObjectResponse>updateMicroRoute(@PathVariable("microRouteId")Long microRouteId
                                                         , @RequestBody MicroRouteRequestDTO request){
        return new ResponseEntity<>(microRouteBusiness.updateMicroRoute(microRouteId,request), HttpStatus.OK);
    }
